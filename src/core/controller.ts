import { LandingPad, RocketState, RocketThrust } from "@/components/RocketSimulator";
...
function torque(alphaDegree: number, thetaDegree: number, thrust?: number) {
    ...
    switch (state.move) {
        case "torque": {
            const targetAngle = Math.PI / 2 + (alphaDegree > 0 ? - thetaDegree : thetaDegree) * Math.PI / 180;
            console.log(angle, targetAngle, angle < targetAngle)
            if (alphaDegree > 0 && angle > targetAngle || alphaDegree < 0 && angle < targetAngle) {
                const angleOfThrust = Math.PI / 2 + alphaDegree * Math.PI / 180
                // console.log(Math.PI / 2, angleOfThrust)
                return {
                    mainThrust: thrust ?? (0.5 / Math.cos(angleOfThrust + angle - Math.PI / 2)), // 0 to 1
                    angleOfThrust,
                };
            }
        ...
        case "counterTorque": {
            ...
            const angularMoment = _rocketState.angularVelocity * _rocketState.inertia
...
export function defaultControlRocket(rocketState: RocketState, landingPad: LandingPad): RocketThrust {
